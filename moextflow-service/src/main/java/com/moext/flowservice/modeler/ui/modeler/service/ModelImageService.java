...
import com.moext.flowservice.modeler.ui.modeler.util.ImageGenerator;
...
public class ModelImageService {
	...
	protected void scaleDiagram(CmmnModel cmmnModel, double scaleFactor) {
		for (Case caseModel : cmmnModel.getCases()) {
			org.flowable.cmmn.model.GraphicInfo graphicInfo = cmmnModel
					.getGraphicInfo(caseModel.getPlanModel().getId());
	...
	protected void calculateWidthForFlowElements(Collection<FlowElement> elementList, BpmnModel bpmnModel,
	...
	protected void calculateWidthForArtifacts(Collection<Artifact> artifactList, BpmnModel bpmnModel,
			GraphicInfo diagramInfo) {
	...
	protected void scaleDecisionService(DecisionService decisionService, DmnDefinition dmnDefinition,
			double scaleFactor) {
		org.flowable.dmn.model.GraphicInfo graphicInfo = dmnDefinition.getGraphicInfo(decisionService.getId());
		scaleDmnGraphicInfo(graphicInfo, scaleFactor);

		for (org.flowable.dmn.model.GraphicInfo decisionServiceDividerInfo : dmnDefinition
				.getDecisionServiceDividerGraphicInfo(decisionService.getId())) {
			scaleDmnGraphicInfo(decisionServiceDividerInfo, scaleFactor);
		}
	}

	protected void scaleDecisions(Collection<DmnElementReference> decisionRefList, DmnDefinition dmnDefinition,
			double scaleFactor) {
	...
	protected void scaleAssociations(List<org.flowable.cmmn.model.Association> associationList, CmmnModel cmmnModel,
			double scaleFactor) {
		for (org.flowable.cmmn.model.Association association : associationList) {
			List<org.flowable.cmmn.model.GraphicInfo> flowList = cmmnModel
					.getFlowLocationGraphicInfo(association.getId());
	...
	protected void scaleCmmnGraphicInfoList(List<org.flowable.cmmn.model.GraphicInfo> graphicInfoList,
			double scaleFactor) {
