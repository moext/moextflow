 ...
 *
 * Keeps imported models mapped by key and id and 'old ids' (model reference ids
 * as stored in the imported json).
 *
 * @author Joram Barrez
 */
public class ConverterContext implements BpmnJsonConverterContext, CmmnJsonConverterContext, DmnJsonConverterContext {
	...
	public void addProcessModel(Model model, String... oldProcessModelIds) {
		...
		// Currently, cmmn -> process task uses this. Process --> process isn't using a
		// reference (hence why not there)
		if (unresolvedProcessModelKeyToCmmnModels != null && !unresolvedProcessModelKeyToCmmnModels.isEmpty()
			...
			for (String key : unresolvedProcessModelKeyToCmmnModels.keySet()) {
				...
				List<String> cmmnModelKeys = cmmnModels.stream().map(cmmnModel -> cmmnModel.getPrimaryCase().getId())
						.collect(Collectors.toList());
				unresolvedProcessModelKeyToCaseModelKey.put(key, cmmnModelKeys);
			}

			handleUnresolvedReferences(modelKey, model, unresolvedProcessModelKeyToCaseModelKey, processKeyToModelMap,
					CmmnStencilConstants.PROPERTY_PROCESS_REFERENCE);
		}
	}
	...
	public void addCaseModel(Model model, String... oldCaseModelIds) {
		...
		// Currently, there is only a caseTask in CMMN, not in BPMN (hence why it's not
		// checked)
		if (unresolvedCaseModelKeyToCmmnModels != null && !unresolvedCaseModelKeyToCmmnModels.isEmpty()
			...
			handleUnresolvedReferences(modelKey, model, unresolvedCaseModelKeyToCaseModelKey, caseKeyToModelMap,
					CmmnStencilConstants.PROPERTY_CASE_REFERENCE);
		}
	}
	...
	protected void handleUnresolvedReferences(String modelKey, Model model,
			Map<String, List<String>> unresolvedReferencesMap, Map<String, Model> keyToModelMap,
			String referenceProperyName) {
		...
		if (unresolvedReferencesMap.containsKey(modelKey)) {
			...
			for (String referencingCaseModelKey : referencingModelKeys) {
				...
				try {
					JsonNode referencingCaseModelJson = objectMapper
							.readTree(referencingCaseModel.getModelEditorJson());
					updateModelReferenceProperties(model, referenceProperyName, referencingCaseModelJson);
					referencingCaseModel.setModelEditorJson(referencingCaseModelJson.toString()); // toString is also
																									// done in the
																									// regular import
	...
	protected void updateModelReferenceProperties(Model model, String referenceProperyName,
			JsonNode referencingCaseModelJson) {
	 ...
	 *
	 * Due to to the order of importing, this is only needed for cmmn <-> bpmn, as
	 * the other model types are imported before them.
	 */

	@Override
	public void registerUnresolvedCaseModelReferenceForCaseModel(String unresolvedCaseModelKey, CmmnModel cmmnModel) {
		// The CmmnModel needs to be passed, as the actual key on the CmmnModel (stored
		// as id) will be typically only be set later.
		unresolvedCaseModelKeyToCmmnModels.computeIfAbsent(unresolvedCaseModelKey, key -> new ArrayList<>())
				.add(cmmnModel);
	}

	@Override
	public void registerUnresolvedProcessModelReferenceForCaseModel(String unresolvedProcessModelKey,
			CmmnModel cmmnModel) {
		// The CmmnModel needs to be passed, as the actual key on the CmmnModel (stored
		// as id) will be typically only be set later.
		unresolvedProcessModelKeyToCmmnModels.computeIfAbsent(unresolvedProcessModelKey, key -> new ArrayList<>())
				.add(cmmnModel);
	}
}
