...
public class IdentityController {
    ...
    /**
     * 删除用户，如果用户不存在则忽略。删除用户后，用户与用户组关系也同时删除。
     * 适用场景：业务系统删除用户时
     * @param request
     * @param userCodeReq
     * @return
    ...
    /**
     * 保存分组，如果分组不存在则新增，如果存在则更新
     * 适用场景：业务系统新增或修改工作流角色时
     * @param request
     * @param groupReq
     * @return
     */
    ...
    /**
     * 删除分组，如果分组不存在则忽略。删除分组后，用户与用户分组关系也同时删除。
     * 适用场景：业务系统删除工作流角色时
     * @param request
     * @param groupIdReq
     * @return
    ...
    @ResponseBody
	public BaseResponse<Boolean> createMembership(HttpServletRequest request, @RequestBody MembershipReq membershipReq) {
    	String validateMsg = RspUtils.validate(membershipReq, MembershipReq.class);
		if(StringUtils.isNotBlank(validateMsg)) {
			return RspUtils.error(validateMsg);
		}
		
	    return RspUtils.success(identityComponent.createMembership(membershipReq.getUserCode(), membershipReq.getGroupId()));
	}	
    
    /**
     * 用户与用户组关系解除，当用户或用户组不存在时忽略
     * 适用场景：业务系统删除单个用户所属角色时
     * @param request
     * @param membershipReq
     * @return
     */
    ...
    @ResponseBody
	public BaseResponse<Boolean> deleteMembership(HttpServletRequest request, @RequestBody MembershipReq membershipReq) {
    	String validateMsg = RspUtils.validate(membershipReq, MembershipReq.class);
		if(StringUtils.isNotBlank(validateMsg)) {
			return RspUtils.error(validateMsg);
		}
		
	    return RspUtils.success(identityComponent.deleteMembership(membershipReq.getUserCode(), membershipReq.getGroupId()));
	}	
    
    /**
     * 按用户全量同步用户-用户组关系
     * 注意：新用户组列表不能为null，可为空，为空时则删除该用户下的所有的用户-用户组关系
    ...
    @ResponseBody
	public BaseResponse<Boolean> syncMembershipByUser(HttpServletRequest request, @RequestBody SyncMembershipByUserReq syncMembershipByUserReq) {
    	String validateMsg = RspUtils.validate(syncMembershipByUserReq, SyncMembershipByUserReq.class);
		if(StringUtils.isNotBlank(validateMsg)) {
			return RspUtils.error(validateMsg);
		}
		
	    return RspUtils.success(identityComponent.syncMembershipByUser(syncMembershipByUserReq.getUserCode(), syncMembershipByUserReq.getNewGroupIdList()));
	}	
    
    /**
     * 按用户组全量同步用户组-用户关系
     * 注意：新用户列表不能为null，可为空，为空时则删除该用户组下所有用户-用户组关系
    ...
    @ResponseBody
	public BaseResponse<Boolean> syncMembershipByGruop(HttpServletRequest request, @RequestBody SyncMembershipByGroupReq syncMembershipByGroupReq) {
    	String validateMsg = RspUtils.validate(syncMembershipByGroupReq, SyncMembershipByGroupReq.class);
		if(StringUtils.isNotBlank(validateMsg)) {
			return RspUtils.error(validateMsg);
		}
		
	    return RspUtils.success(identityComponent.syncMembershipByGroup(syncMembershipByGroupReq.getGroupId(), syncMembershipByGroupReq.getNewUserCodeList()));
	}
