...
public class TestTodoListPageReq {

	//测试常用赋值
	@Test
	public void testGenExampleNormal() throws ParseException {
		TodoListPageReq req = new TodoListPageReq();
	...
	//测试边界
	@Test
	public void testGenExampleEdge() throws ParseException {
		TodoListPageReq req = new TodoListPageReq();
		TodoTaskExample example = req.genExample();
	...
	public void testSort() {
		...
		assertEquals("is_del =", example.getOredCriteria().get(0).getAllCriteria().get(0).getCondition());
		assertEquals(false, example.getOredCriteria().get(0).getAllCriteria().get(0).getValue());
	}
